...
export class CacheService {
    ...
    async cacheWiki(ipfs_hash: string): Promise<Boolean> {
        ...
        return this.ipfs.client().add(Buffer.from(wiki, 'utf8'))
            .then(res => {
                if (ipfs_hash == res[0].hash)
                    console.log(`CACHE: Cached ${ipfs_hash} to local IPFS node from MySQL`);
                else
                    console.log(`CACHE: WARNING: MySQL entry for ${ipfs_hash} does not match generated hash of ${res[0].hash}`);
            });
    }
