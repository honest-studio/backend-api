...
commander
  ...
  .description('Deindex pages from Wikipedia due to duplicate content')
  .usage('[OPTIONS]...')
...
const DELETED_PAGE_NOTE = '|EN_WIKI_IMPORT_DELETED|';

export const logYlw = (inputString: string) => {
    return console.log(chalk.yellow.bold(inputString));
}

export const WikipediaImportDeindexer = async (inputString: string) => {
        ...
        const fetchedArticles: any[] = await theMysql.TryQuery(
            `
                ...
                WHERE art.id between ? and ?
                    ...
                    AND art.page_note=? 
            `,
            [currentStart, currentEnd, '|EN_WIKI_IMPORT|']
        );

        for await (const artResult of fetchedArticles) {
            try{
                await WikipediaImportDeindexer(artResult.concatted);
            }
            catch (err){
