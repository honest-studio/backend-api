...
export const WikiNewPagesSince = async (api_start: number, api_end: number) => { 
    ...
    const recent_edits_url = `${wikiMedia}action=query&list=logevents&lelimit=${RC_LIMIT}&format=json&letype=create&lenamespace=0&ledir=newer&lestart=${api_start}&leend=${api_end}`;
    ...
    // Try to remove redirects
    parsed_body = parsed_body.map(event_obj => {
        if(event_obj.comment && event_obj.comment.search(/redirect/gimu) >= 0) return null;
        else return event_obj;
    }).filter(obj => obj);
...
//     // Check recent edits on Wikipedia
//     console.log(chalk.bold.yellow(`Checking for new pages ✍️ ...`));
//     const wikiMedia = `https://${LANG_CODE}.wikipedia.org/w/api.php?` // Default WikiMedia format
